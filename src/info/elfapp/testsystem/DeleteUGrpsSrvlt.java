 ...
 */
@WebServlet("/DeleteUGrpsSrvlt")
public class DeleteUGrpsSrvlt extends HttpServlet {
    private TSystem tsystem;

    public DeleteUGrpsSrvlt() {
        super();
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        JSONObject res = new JSONObject();
            if(request.getParameter("id")!=null){
                try {
                    UsersGroups q = DAO.getInstance().getUsrGroupsDAO().getObjById(Long.decode(request.getParameter("id")));
                    DAO.getInstance().getUsrGroupsDAO().deleteObj(q);
                    res.put("res","ok");
